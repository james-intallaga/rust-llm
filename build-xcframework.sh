#!/usr/bin/env bash
#
# Forge SDK xcframework build script
# Builds llama.cpp + mtmd (multimodal) with Metal GPU support
#
# Copyright (c) 2026 AMMA AI Intallaga Tech. Built on llama.cpp.
#
# Usage: Run from the amma-forge-sdk root directory
#   ./build-xcframework.sh
#
# The script uses the llama.cpp submodule and outputs to llama.xcframework/
#
...
# Check that the submodule exists
...
# Change to llama.cpp directory for the build
cd "${LLAMA_CPP_DIR}"
...
GGML_METAL_EMBED_LIBRARY=ON  # Embed Metal shaders in the library
GGML_BLAS_DEFAULT=ON
GGML_OPENMP=OFF

COMMON_C_FLAGS="-O3 -DNDEBUG -Wno-macro-redefined -Wno-shorten-64-to-32 -Wno-unused-command-line-argument -ffile-prefix-map=${SCRIPT_DIR}=."
COMMON_CXX_FLAGS="-O3 -DNDEBUG -Wno-macro-redefined -Wno-shorten-64-to-32 -Wno-unused-command-line-argument -ffile-prefix-map=${SCRIPT_DIR}=."
...
## Clean up previous builds (inside llama.cpp submodule)
rm -rf build-apple
rm -rf build-ios-sim
...
setup_framework_structure() {
    ...
    # Copy mtmd headers for multimodal support
    cp tools/mtmd/mtmd.h           ${header_path}
    cp tools/mtmd/mtmd-helper.h    ${header_path}

    # Create module map with Metal and mtmd support
...
combine_static_libraries() {
    ...
    # Create dynamic library with Metal framework
    ...
    xcrun -sdk $sdk clang++ -dynamiclib \
        ...
        -framework Foundation -framework Accelerate -framework Metal -framework MetalPerformanceShaders \
...
# Create a release XCFramework without developer-local debug symbols.
echo "Creating XCFramework..."

# Remove old xcframework from project root if it exists
rm -rf "${SCRIPT_DIR}/llama.xcframework"

xcodebuild -create-xcframework \
    -framework $(pwd)/build-ios-sim/framework/llama.framework \
    -framework $(pwd)/build-ios-device/framework/llama.framework \
    -framework $(pwd)/build-macos/framework/llama.framework \
    -output "${SCRIPT_DIR}/llama.xcframework"

echo ""
echo "=========================================="
echo "XCFramework built successfully!"
echo "Output: ${SCRIPT_DIR}/llama.xcframework"
echo ""
echo "Features:"
echo "  - Metal GPU acceleration: ENABLED"
echo "  - Multimodal (mtmd): ENABLED"
